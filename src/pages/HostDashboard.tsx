...
// Sample mock data for demo host view
...
const INITIAL_DEMO_PLAYERS: MockPlayer[] = [
  ...
  { id: 'p4', name: 'Chris Evans', table_label: 'Table 2', player_code: 'V3P-6HK', active: false, hand: [], hasActedInWindow: false }
];
...
export const HostDashboard: React.FC = () => {
  ...
  // Tab navigation: 'overview' | 'catalog' | 'history'
  const [activeTab, setActiveTab] = useState<'overview' | 'catalog' | 'history'>('overview');

  // Room data state
  const [players, setPlayers] = useState<MockPlayer[]>(INITIAL_DEMO_PLAYERS);
  const [cards, setCards] = useState<Card[]>(INITIAL_DEMO_CARDS);
  const [lastActionText, setLastActionText] = useState<string | null>('Granted Shield of Faith to Sam Taylor');
  ...
  const [selectedGrantCardId, setSelectedGrantCardId] = useState(INITIAL_DEMO_CARDS[0].id);
  ...
  const handleIssuePermission = () => {
    setWindowOpen(true);
    // Mark target players as not having acted yet
    setPlayers((prev) =>
      prev.map((p) => {
  ...
  const handleCloseWindow = () => {
    if (permAction === 'draw') {
      // Perform auto-draw for players who didn't act
      setPlayers((prev) =>
        prev.map((p) => {
          if (!p.active) return p;
          if (
  ...
  const handleGrantCard = (playerId: string) => {
    ...
    setLastActionText(`Granted ${cardToGrant.title} to ${targetP?.name}`);
    setShowGrantModal(false);
  };

  // Host Revoke Card
  const handleRevokeCard = (playerId: string, heldCardId: string) => {
    setPlayers((prev) =>
      prev.map((p) =>
        p.id === playerId
  ...
  const handleRemovePlayer = (playerId: string) => {
    setPlayers((prev) =>
      prev.map((p) => (p.id === playerId ? { ...p, active: false } : p))
    );
  ...
  const handleRestorePlayer = (playerId: string) => {
    setPlayers((prev) =>
      prev.map((p) => (p.id === playerId ? { ...p, active: true } : p))
    );
  ...
  // Total Card Weight sum for probabilities
  const totalWeight = cards.reduce((sum, c) => sum + (c.active ? c.weight : 0), 0);

  // Reset Room Data / End Session
  const handleResetRoom = async () => {
    if (!window.confirm('Are you sure you want to end this session and reset all room data? All player hands and active history for this room will be cleared.')) {
      return;
    }
    if (isDemoMode) {
      setPlayers([]);
    ...
    try {
      const { error } = await supabase.rpc('reset_room', { p_room_code: roomCode });
      if (error) {
        alert(error.message);
      } else {
        setPlayers([]);
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col pb-24">
      ...
      <header className="sticky top-0 z-40 bg-slate-900/90 backdrop-blur-md border-b border-slate-800 px-4 sm:px-6 py-3 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Trash2 className="w-3.5 h-3.5" /> End Session / Reset
          </button>
        </div>
      </header>
        ...
        {activeTab === 'catalog' && (
          <div className="space-y-6">
            <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex items-center justify-between">
              <div>
                <h2 className="text-base font-bold text-white mb-1">Card Catalog & Storage Sync</h2>
                <p className="text-xs text-slate-400">
                  Manage cards synced from the storage bucket and adjust draw weights.
                </p>
              </div>
              <button
                onClick={() => alert('Catalog synced with storage bucket!')}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl flex items-center gap-2"
              >
                <RefreshCw className="w-4 h-4" /> Sync Bucket
              </button>
            </div>

            {/* Card Weights & Probability Table */}
